...
import cofferfile
from naclfile.zstd import CParameter, DParameter
from naclfile.tar import TarFile, open as tar_open
...
def test_buffer_nacl_file(random_path, random_name, chunk_size, file_size):
    ...
    with TarFile(dataf, mode='wb', secret_key=key, level_or_option=level_or_option, chunk_size=chunk_size) as ff:
...
def test_buffer_nacl_open(random_path, random_name, chunk_size, file_size):
    ...
    with tar_open(dataf, mode='wb', secret_key=key, level_or_option=level_or_option, chunk_size=chunk_size) as ff:
        ff.add(dataf1,'file1.data')
        ff.add(dataf2,'file2.data')
...
def test_bad(random_path, random_name):
    ...
    with TarFile(dataok, mode='wb', secret_key=key) as ff:
        assert repr(ff).startswith('<TarZstdNacl')

    with pytest.raises(ValueError):
        with TarFile(dataf, mode='wbt', secret_key=key) as ff:
            ff.write(data)

    with pytest.raises(ValueError):
        with TarFile(dataf, mode='zzz', secret_key=key) as ff:
            ff.write(data)

    with pytest.raises(FileNotFoundError):
        with TarFile(None, mode='wb', secret_key=key) as ff:
            ff.write(data)

    with pytest.raises(FileNotFoundError):
        with TarFile(dataf, secret_key=key) as ff:
            data = ff.read()

    with pytest.raises(ValueError):
        with tar_open(dataf, mode='wbt', secret_key=key) as ff:
            ff.write(data)

    with pytest.raises(ValueError):
        with tar_open(dataf, mode='wb', secret_key=key, encoding='utf-8') as ff:
            ff.write(data)

    with pytest.raises(ValueError):
        with tar_open(dataf, mode='wb', secret_key=key, errors=True) as ff:
            ff.write(data)

    with pytest.raises(ValueError):
        with tar_open(dataf, mode='wb', secret_key=key, newline='\n') as ff:
            ff.write(data)

    with pytest.raises(TypeError):
        with tar_open(None, mode='wb', secret_key=key) as ff:
            ff.write(data)

    with pytest.raises(ValueError):
        with tar_open(dataf, mode='wb', secret_key=None) as ff:
            ff.write(data)
